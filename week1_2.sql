...
rguktuser@preethi:~$ cat> file2
user@preethi:~$ cp file1 file2

mv: Move files or rename files
• Syntax: mv [OPTION]source destination
user@preethi:~$ mv file1 file2
...
user@preethi:~$ rm file2

mkdir: Make a directory
• Syntax: mkdir [OPTION] directory
user@preethi:~$ mkdir dir3
user@preethi:~$ rmdir dir1

cd: Change directory
• Syntax: cd [OPTION] directory
user@preethi:~$ cd dir3

pwd: Print the present working directory
• Syntax: pwd [OPTION]
user@preethi:~/dir3$ pwd dir3
/home/user/dir3

ls: List directory contents
• Syntax: ls [OPTION] [FILE]
user@preethi:~$ ls dir3

which: Locate a command
• Syntax: which [-a] filename
user@preethi:~$ which [-a] file1

su: Change user-id or become super-user.
• Syntax: su [options] [username]
• Example: Change user-id to ‘user1’ (if it exists)
user@preethi:~$ su user1
su: user user1 does not exist

user@preethi:~$ sudo ls/usr/local/protected
...
user@preethi:cat>grepfilter.txt
welcome to ubuntu
...
user@preethi:~$ grep "rgukt" grepfilter.txt
rgukt rajiv gandhi university konwledge technology


-v: This prints out all the lines that do not matches the pattern
user@preethi:~$ grep -v "unix" grepfilter.txt
welcome to ubuntu
rgukt rajiv gandhi university konwledge technology
...
-v: This prints out all the lines that do not matches the pattern
user@preethi:~$ grep -v "this" grepfilter.txt
welcome to ubuntu
hii is linux/unix commands
rgukt rajiv gandhi university konwledge technology

Creating a file
user@preethi:~$ cat>monthfilter.txt
august 
september
march
june
...
user@preethi:~$ sort -M monthfilter.txt
march
june
july
august
...
user@preethi:~$ cat>numberfilter.txt
88 45 3 2 89

-n: Sorts a file numerically (interprets data as numbers).
user@preethi:~$ sort -n nufile1.txt
01
2
3
4
...
user@preethi:~$ sed 's/rgukt/RGUKT/' grepfilter.txt
welcome to ubuntu
hii is linux/unix commands
RGUKT rajiv gandhi university konwledge technology

user@preethi:~$ sed 's/o/**/g' grepfilter.txt
welc**me t** ubuntu
hii is linux/unix c**mmands
rgukt rajiv gandhi university k**nwledge techn**l**gy


user@preethi:~$ cat>employee.txt
...
user@preethi:~$ awk '{print $1,$3}' employee.txt
venky id
vinay id
mark id
john id
